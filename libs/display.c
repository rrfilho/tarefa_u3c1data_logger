...
static bool display_update() {
    ...
    draw_string("Data Logger", 8, 6);
    draw_string(_status, 10, 16);
    draw_string(_samples, 10, 28);
    draw_string(_message, 10, 41);
    send_data();
    return true;
}
...
void display_set_status(char* status) {
    _status = status;
}

void display_set_samples(int samples) {
    static char message[32];
    sprintf(message, "Linhas: %d", samples);
    _samples = message;
}

void display_set_message(char* message) {
    _message = message;
}